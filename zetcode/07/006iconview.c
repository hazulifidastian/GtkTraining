...
GtkTreeModel * init_model( void ) {
    ...
    p1 = gdk_pixbuf_new_from_file( "recycle.png", &err );
    p2 = gdk_pixbuf_new_from_file( "recycle.png", &err );
    p3 = gdk_pixbuf_new_from_file( "recycle.png", &err );
    p4 = gdk_pixbuf_new_from_file( "recycle.png", &err );
    ...
    for( i; i < 50; i++ ) {
        ...
        gtk_list_store_set( list_store, &iter, COL_DISPLAY_NAME,
            "recycle", COL_PIXBUF, p1, -1 );

        gtk_list_store_append( list_store, &iter );
        gtk_list_store_set( list_store, &iter, COL_DISPLAY_NAME,
            "recycle", COL_PIXBUF, p2, -1 );

        gtk_list_store_append( list_store, &iter );
        gtk_list_store_set( list_store, &iter, COL_DISPLAY_NAME,
            "recycle", COL_PIXBUF, p3, -1 );

        gtk_list_store_append( list_store, &iter );
        gtk_list_store_set( list_store, &iter, COL_DISPLAY_NAME,
            "recycle", COL_PIXBUF, p4, -1 );
    }
